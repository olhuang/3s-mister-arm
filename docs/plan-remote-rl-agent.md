...
- [ ] Phase 11: move beyond DQN/BC into an event-aware actor-critic line
  ...
  - [ ] Phase 11C-3 movement-subaction/state-phase balance: the remaining collapse is inside the movement-defense family, especially `back`. Before live inference, add diagnostics and training controls that split actor updates by mask phase / movement subaction so useful walk/guard/jump rows are preserved without allowing `back` to dominate every non-attack state.
  - [ ] Phase 11D probe actor-critic inference: add `--policy actor-critic` to `rl_probe_server.py`, load `actor_critic` artifacts, apply the same valid-action mask as BC/DQN, and support stochastic sampling controls (`temperature`, top-k/top-p) plus value diagnostics. No live deploy until 11C local action-family stats beat 10C/10D.
