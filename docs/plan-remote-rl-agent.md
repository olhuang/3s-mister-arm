...
- [ ] Combat event attribution Phase 9: add opt-in trainer use of high-confidence event labels only after move-family validation passes
  ...
  - [ ] Phase 9C-2 prefer-event-action mode: after reward-shaping MVP passes, allow high-confidence source events to create direct replay experiences with state from `start_decision_id`, action from event policy metadata, reward from event outcome, and next_state from `end_decision_id`.
