...
- [ ] Phase 10: promote behavior cloning from a DQN warm-start helper into the primary imitation-policy line
  ...
  - [ ] Phase 10B BC actor inference: support `policy=bc` in the probe actor path, including valid-action masking, softmax temperature, top-k sampling, and deterministic danger-state overrides for low/projectile threats.
