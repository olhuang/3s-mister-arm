...
- [ ] Combat event attribution Phase 9: add opt-in trainer use of high-confidence event labels only after move-family validation passes
  ...
  - [x] Phase 9B trainer reader: add explicit `--combat-event-logs`, `--combat-event-training-mode off|validate|reward-shaping`, `--combat-event-reward-profile safe-v1`, and `--combat-event-reward-scale` flags. Defaults remain off and preserve existing training behavior without event flags; `prefer-event-action` remains Phase 9C.
  ...
  - [ ] Phase 9C prefer-event-action mode: after reward-shaping MVP passes, allow high-confidence source events to create direct replay experiences with state from `start_decision_id`, action from event policy metadata, reward from event outcome, and next_state from `end_decision_id`.
