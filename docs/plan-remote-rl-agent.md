...
- [ ] Phase 12: hierarchical tactical-state policy line
  ...
  - [ ] Phase 12F event-feedback refinement: use combat-event outcomes to score intents, not only actions. Examples: `approach` followed by getting hit is bad approach timing; `low_guard` blocking low attacks is successful defense; `anti_air` hitting a jump-in is successful reaction; `punish` landing after opponent recovery is successful punish.
  - [ ] Phase 12G promotion gates: a hierarchical candidate must improve live tactical stats over Phase 11, avoid a single-intent collapse, preserve movement/defense competence, and keep transition/event training compatibility clean before replacing existing DQN/BC/actor-critic baselines.
