...
- [ ] Combat event attribution Phase 9: add opt-in trainer use of high-confidence event labels only after move-family validation passes
  ...
  - [ ] Phase 9D event-aware sampling: add optional rare-event oversampling for punish, throw, projectile parry/clash, DP punish/block, SA, and other validated low-frequency labels only after the reward-shaping path is stable.
