...
- [ ] Phase 11: move beyond DQN/BC into an event-aware actor-critic line
  ...
  - [ ] Phase 11D probe actor-critic inference: add `--policy actor-critic` to `rl_probe_server.py`, load `actor_critic` artifacts, apply the same valid-action mask as BC/DQN, and support stochastic sampling controls (`temperature`, top-k/top-p) plus value diagnostics. Candidate `actor-critic-awac-movement-balanced-human-11c3-v2` is the first local actor-critic candidate worth a live gate once inference is implemented.
