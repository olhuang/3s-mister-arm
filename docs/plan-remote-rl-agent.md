...
- [ ] Phase 11: move beyond DQN/BC into an event-aware actor-critic line
  ...
  - [ ] Phase 11C offline AWAC/AWR trainer: train actor logits and value head from the 11B dataset. Actor loss should weight `-log pi(a|s)` by clipped/temperature-scaled positive advantage; value loss should regress event returns; entropy and action-mask-aware diagnostics must be recorded. Start from human-only paired logs before mixing automated data.
