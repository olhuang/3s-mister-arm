...
- [ ] Combat event attribution Phase 9: add opt-in trainer use of high-confidence event labels only after move-family validation passes
  ...
  - [ ] Phase 9C-2 event-aware replay filtering: add opt-in filtering/downsampling for unlabeled passive movement rows in event-aware reward-shaping, initially targeting `event-damage-v1`. Keep all attack/source-event rows, all defense-labeled rows, all projectile-threat response rows, all damage rows, and all done/boundary rows; downsample only zero-reward `back`/`forward`/`guard-*`/jump-start rows with no combat label, no incoming threat, no HP/stun delta, and no source event. Default policy remains `keep`.
