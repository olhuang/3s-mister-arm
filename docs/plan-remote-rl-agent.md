...
- [ ] Combat event attribution Phase 6a: implement edge-triggered contact-to-attack/projectile/throw matching with consumed HP/stun deltas, trade handling, confidence, and attribution failure events
  ...
  - [x] Phase 6a-2 recent source continuation: if a new HP/stun delta has no active source candidate, C now conservatively reconnects it to the most recent same-direction confirmed attack/projectile attribution within a short window (8 frames for attack, 12 for projectile), provided no opposite-direction confirmed source intervened. Continuation rows do not create new punish candidates and throws are not continued.
  - [x] Phase 6a-3 finalized attack rescue: if a strong HP/stun/damage edge has no projectile, throw, or active attack candidate, C can reconnect it to the newest same-side non-projectile attack that was just finalized as clean whiff or superseded unknown within an 18-frame rescue window. Active/newer sources still win first, and projectile-like parents are excluded so this only rescues orphan strike damage such as late air-normal contact or normals that overlap an existing projectile.
- [ ] Combat event attribution Phase 6b: implement defense result emission with intended action, actual guard state at contact, target_state, wakeup context, block_possible, confidence, and failure reasons
  ...
  - [x] Phase 6b-2 dual-whiff weak-edge suppression: attack-sourced attribution rows are no longer recorded when the target is also attacking and the only evidence is weak `contact_state` / `hit_stop` without HP/stun/damage/block/parry/throw/projectile-clash evidence. This keeps simultaneous whiffs in `CER W` instead of polluting `CDRX U`.
  - [x] Phase 6b-3 post-hit jump weak-edge suppression: the same weak attack-sourced attribution gate now covers airborne targets, preventing a clean hit followed by an immediate jump from adding an extra `CDRX U` when only residual `contact_state` / `hit_stop` evidence remains.
  - [x] Phase 6b-4 weak attack-only suppression: live retest showed post-hit jump-start timing could still occur before the airborne flag. The C gate now suppresses all attack-sourced weak `contact_state` / `hit_stop` attribution rows when no strong outcome evidence exists, independent of target state.
  - [x] Phase 6b-5 attack lifecycle contact-resolve: close-range non-projectile attack events that saw target contact/damage now finalize as `result=contact, finalize_reason=contact_resolved` once the actor leaves attack state, instead of staying active until `basic_unknown_timeout` and later incrementing `CER U`. This is only a lifecycle cleanup; hit/block truth remains in attribution/defense-result rows.
  - [x] Phase 6b-6 attack-start debounce widening: same-signature attack-start debounce was widened to 24 frames so one close-range physical attack is not double-counted when a start edge is followed later by a contact/Attack_Counter edge. Fast repeated same-move chains may remain intentionally merged as one conservative event.
  - [x] Phase 6b-7 Ryu engine-owned attack starts: Ryu attack events now require engine routine/current-attack start attribution and no longer use `Attack_Counter` alone to open new attack events. `Attack_Counter`, hit-stop, HP/stun, block, projectile, and throw signals remain evidence/finalization inputs. Attack journal rows now include raw routine/current-attack/KW fields for Ryu-vs-Ryu state-machine validation.
  - [x] Phase 6b-8 Ryu lifecycle cleanup: throw routine starts no longer open attack events, and active attacks that already saw target contact/damage resolve as `contact_resolved` before supersede or timeout can turn them into unknown. This keeps throw statistics in the throw ring and reduces false `rollover_unknown` / `timeout_unknown` rows without guessing outcomes that lack contact evidence.
...
- [ ] Combat event attribution Phase 7: roll out transition schema v4 compact summaries plus `combat_event_schema_version=1` event journal export inside the same transition batch/envelope
  ...
  - [x] Phase 7A-3 per-frame combat context fallback: combat event rings now continue to update from a lightweight frame context even when no active decision ledger entry is available, so event capture is not fully dependent on RL decision-span lifetime. Active ledger rows still receive transition/reward accumulation; the frame context only feeds the combat event journal. Ryu non-projectile special contact can also open an unknown-strength fallback source when damage arrives while no active source exists, reducing `no_source_candidate` rows for cases such as opponent `routine2=17` shoryuken damage.
  - [x] Phase 7A-4 Ryu `R2=2` taxonomy/source correction: source-table review confirmed Ryu `R1=4/R2=2` is common `Attack_02000` from the Ryu MP branch, not a throw. Live MP-only logs showed it as an air-MP follow-up damage window. C now stops mapping Ryu `R2=2` to throw, keeps throw startup on `R2=14/15`, and can open a damage-triggered Ryu air-MP continuation source when an `R2=2` HP/stun delta arrives without an active source.
  - [x] Phase 7A-4a Ryu `R2=2` fallback branch fix: fresh MP-only live logs still showed two self-side `no_source_candidate` rows during `R1=4/R2=2` damage because the damage fallback branch tested the Ryu special path inverted and never reached the air-MP continuation branch. The fallback now checks Ryu specials first, then applies the Ryu-only `R2=2` continuation source when the special mapper does not match.
  - [x] Phase 7A-5 Ken engine attribution support: Ken now uses the same engine-owned attack/throw start path as Ryu, with source-backed routine mapping for Hadouken, Shoryuken, Tatsumaki, Air Tatsumaki, Shoryureppa, Shinryuken, and Shippu Jinraikyaku. Ken does not inherit the Ryu-only `R2=2` continuation fallback; analyzers/probe metadata map Ken `212x` action IDs back to the existing generic fireball/shoryuken/tatsu move names.
...
- [ ] Combat event attribution Phase 9: add opt-in trainer use of high-confidence event labels only after move-family validation passes
  - [ ] Phase 9A data recollection gate: collect fresh paired transition/event logs before any event-aware DQN training. Old transition-only logs remain valid for baseline training and comparison, but they must not be used as event-reward training data.
  - [ ] Phase 9A collection contract: every training capture must write both sibling files from the same batch envelope, e.g. `*-transitions.ndjson` and `*-events.ndjson`, with `rl-agent-export-combat-events=on` and probe `--combat-event-log` enabled. The transition file must contain zero `combat_event_schema_version` rows, and the event file must contain only combat event journal rows.
  - [ ] Phase 9A schema gate: accept only transition schema 9+ rows paired with `combat_event_schema_version=1` rows until a later schema migration is explicitly documented. Reject mixed or missing event schema rows in event-aware trainer modes.
  - [ ] Phase 9A minimum recollection curriculum:
    - Ryu vs Ryu sanity: LP/MP/HP/LK/MK/HK, fireball, Shoryuken, Tatsumaki, throw, projectile clash, parry, and whiff/punish examples.
    - Ken vs Ryu sanity: LP/MP/HP/LK/MK/HK, Hadouken, Shoryuken, Tatsumaki, Air Tatsumaki, throw, and Ken SA1/SA2/SA3 where selectable.
    - Side-symmetry sanity: at least one self-controlled and one opponent/CPU-controlled sample for attack, projectile, throw, attribution, and punish rows.
    - Negative/safety samples: blocked DP, whiffed throw, clean whiff, projectile expired safely, projectile parried, projectile clash, and round/pause/session boundary flush.
  - [ ] Phase 9A analyzer gate: run `tools/analyze_rl_combat_events.py <events> --transition-log <transitions>` for every candidate dataset and record event id ordering, missing refs, transition join coverage, projectile counter checks, unknown/no_source counts, move stats, and per-side source/target splits.
  - [ ] Phase 9A training eligibility gate: event-aware reward shaping may only use event families whose unknown/no_source rate is below the documented threshold, initially 10% per event family, or whose unknown rows are explicitly excluded from reward. Unresolved `unknown`, `no_source_candidate`, schema-mismatch, duplicate, missing-ref, or journal-overflow episodes are analysis-only and must not shape rewards.
  - [x] Phase 9B-0 trainer validation reader: `tools/train_dqn_learner.py` now accepts `--combat-event-logs` with `--combat-event-training-mode validate`, parses schema-v1 combat event journals through `tools/rl_combat_event_training.py`, scans transition logs for accidental combat-event rows, checks event refs and transition start/decision joins, and records diagnostics in model metadata without changing rewards or DQN features.
  - [x] Phase 9B trainer reader: add explicit `--combat-event-logs`, `--combat-event-training-mode off|validate|reward-shaping`, `--combat-event-reward-profile safe-v1`, and `--combat-event-reward-scale` flags. Defaults remain off and preserve existing training behavior without event flags; `prefer-event-action` remains Phase 9C-2.
  - [x] Phase 9B reward-shaping MVP: join event rows to transition rows by `(run_id, episode_id, start_decision_id)` and apply small, opt-in self-side event reward adjustments, using high-confidence attribution rows first, while keeping DQN feature names unchanged. Event fields are labels/metadata, not model inputs.
  - [x] Phase 9B-2 reward aggregation/cap: `safe-v1` now groups same-source attribution rows and caps duplicate same-outcome rewards, so one source event with multiple HP/stun delta ticks cannot receive repeated hit/block rewards. Metadata records attribution rows seen, grouped source events, capped duplicate rows, and grouped HP/stun totals.
  - [x] Phase 9C defensive reward-shaping: `safe-v1` now joins high-confidence opponent-source attribution rows by `(run_id, episode_id, decision_id)` when `target_side=self`, applies conservative defense rewards (`hit`/`thrown` negative, `blocked`/`blocked_chip`/`parry`/`evaded` positive or neutral), keeps raw `unknown` at zero reward, and caps duplicate same-source same-result defense rows across decisions.
  - [x] Phase 9C-1 event-damage-v1 reward profile: reward-shaping mode now has an opt-in event-primary profile that disables transition HP-delta reward, uses high-confidence combat-event attribution `target_hp_delta` grouped by source event for attack/projectile/throw/defense damage, keeps DQN features unchanged, and leaves unknown/no-source/unresolved rows at zero reward.
  - [x] Phase 9C-2 event-aware replay filtering: add opt-in filtering/downsampling for unlabeled passive movement rows in event-aware reward-shaping, initially targeting `event-damage-v1`. Keep all attack/source-event rows, all defense-labeled rows, all projectile-threat response rows, all damage rows, spacing-success movement rows, and all done/boundary rows; downsample only zero-reward `back`/`forward`/`guard-*`/jump-start rows with no combat label, no incoming threat, no HP/stun delta, no source event, and no spacing success. Default policy remains `keep`.
    - 2000-step `event-damage-v1 + downsample 0.20` validation preserved protected rows (`protected_dropped=0`) but still produced a defensive policy (`attack_rate=0.1%`, `defense_rate=86.5%` on the same-log 5000-row compare). Treat filtering as a replay-composition tool, not a sufficient trainer-quality fix by itself.
  - [ ] Phase 9C-3 prefer-event-action mode: after reward-shaping plus replay filtering pass, allow high-confidence source events to create direct replay experiences with state from `start_decision_id`, action from event policy metadata, reward from event outcome, and next_state from `end_decision_id`.
  - [x] Phase 9D event-aware source-family batch sampling: add an opt-in DQN batch sampler for `event-damage-v1` reward-shaping that groups experiences into `attack`, `projectile`, `defense`, `punish_throw`, `movement`, and `unlabeled_passive` pools. Default remains off. `balanced-v1` records pool/target diagnostics and can keep `unlabeled_passive=0.00` while still preserving movement support through a dedicated movement pool. On the 30-round Ryu/Ken paired log, attack-heavy ratios plus `action-start-v1` valid-action masking improved same-log compare from base `attack_rate=5.6%` to `13.0%` and reduced defense drift versus the unmasked attack-heavy candidate (`defense_rate=61.1%` vs `76.4%`), but still needs live validation before promotion.
  - [x] Phase 9E delayed movement credit foundation: add opt-in `--combat-event-movement-credit delayed-v1` to allocate a capped/discounted fraction of resolved combat-event reward back to recent `forward` / `back` / jump-start movement experiences, with metadata for applied anchors, rows, actions, reasons, and skips. Default remains off. First validation on the 30-round Ryu/Ken paired log showed the v1 gating is safe but very conservative and did not change same-log policy distribution versus the Phase 9D best candidate; treat this as infrastructure, not a promotion candidate.
  - [x] Phase 9E-2 movement-credit dry-run diagnostics: add `tools/analyze_rl_movement_credit.py` to replay paired transition/event logs without training, compare movement-credit scales, print credited rows/reasons/actions/examples, report guard-cost candidates, and catch trainer drift. The dry-run found and fixed a tiny cross-episode delayed-credit leak; revised scale `0.35` stats on the 30-round Ryu/Ken log are `195` credited movement rows from `102` anchors, net `+0.128`, with zero cross-episode credits.
  - [x] Phase 9F low-defense trainer hook: add opt-in event-log low attack defense shaping and margin loss. Source events whose normalized engine/policy action is `crouch-lk`, `crouch-mk`, or `crouch-hk` are treated as low threats. If `target_side=self` and the low attack hits, `guard-stand`, `back`, `forward`, or an unlabeled/no-action row can receive an extra penalty. If the low attack is blocked by `guard-crouch`, the row can receive a small bonus. A separate valid-action-masked margin can require `Q(guard-crouch) > Q(guard-stand/back/forward)` in low-threat rows. Defaults remain off and DQN features remain unchanged.
  - [x] Phase 9F-1 opponent normal stance label fix: Ryu/Ken opponent-side normals now use previous-frame ordinary routine context to classify `R1=4/R2=0 + AK/KW=<button>` as `crouch_normal` when the attack starts immediately after crouch-start/crouch/turn-crouch/crouch-guard context. This fixes low-defense captures where Ken down LK/MK/HK were visible in live play but emitted as `stand-lk`, `stand-mk`, or `stand-hk`.
  - [ ] Phase 9F data gate: collect paired logs that deliberately include Ryu/Ken `crouch-lk`, `crouch-mk`, and `crouch-hk` hitting and being blocked. Logs captured before Phase 9F-1 can under-report low-source events; re-record after the stance-label fix before using the data for low-defense reward or margin shaping.
- [ ] Phase 10: promote behavior cloning from a DQN warm-start helper into the primary imitation-policy line
  - [x] Phase 10A event-weighted BC trainer: `--training-mode bc` now supports opt-in `--bc-event-weighting event-weighted-v1` with paired combat-event validation. The trainer derives a combat-event reward adjustment per labeled BC row, maps positive/negative outcomes into clamped cross-entropy sample weights, and records weighted-row, clamp, raw-adjustment, reward, and per-label weight diagnostics. Defaults remain off; DQN reward shaping is unchanged.
  - [x] Phase 10A-1 BC label balancing: add opt-in `--bc-label-balance off|inverse-sqrt|inverse-frequency` with min/max per-label multipliers so event-weighted BC is not dominated by high-volume `forward`/`back`/light-normal labels before Phase 10B stochastic inference exists.
  - [x] Phase 10B BC actor inference: `rl_probe_server.py` now accepts `--policy bc`, loads `policy=bc` model manifests that use the shared `dqn` network payload, applies a BC-specific valid-action mask defaulting to `action-start-v1`, and selects actions with top-k softmax sampling (`--bc-temperature`, `--bc-top-k`). Optional `--bc-deterministic-danger` can force the best guard/back action for close grounded attack or urgent incoming-projectile rows. Local sanity on the human-only 10B model changed first-5000-row masked action selection from hard argmax `forward=3855/back=1144` to sampled `forward=2396/back=2119/stand-lp=233/guard-crouch=214`, so live validation is the next gate.
  - [ ] Phase 10C event-aware BC negative/margin objectives: add explicit contrastive losses for low defense, projectile defense timing, punish opportunities, and bad demonstrations instead of relying only on sample downweighting. Phase 10B live gate failed (`bc-event-weighted-balanced-human-10b-v1` only shuffled forward/back and LP), so 10C now starts with stronger BC class balancing before adding new loss terms. First candidate `model/bc-event-weighted-balanced-human-10c-v1` uses inverse-frequency label balancing and stronger event weights; local first-5000-row sanity at temp 0.8/top-k 5 gives `back=1543`, `forward=1462`, `guard-crouch=1125`, `stand-hp=371`, `stand-lp=157`, `shoryuken-hp=127`, `shoryuken-mp=59`, `throw=48`.
    - [x] Phase 10C-1 positive-event family margin: add `--bc-family-margin positive-event-v1`, an opt-in BC margin objective that uses positive combat-event reward adjustments to require meaningful attack/projectile/throw/guard-crouch labels to outrank generic negative actions (`forward,back,guard-stand,guard-crouch` by default). Candidate `model/bc-event-weighted-family-margin-human-10c-v2` trained from the same human-only paired log with 233 margin-eligible rows; first-5000-row sanity at temp 0.8/top-k 8 gives `back=1438`, `forward=1395`, `guard-crouch=1000`, `crouch-mk=216`, `stand-hp=203`, `guard-stand=203`, `throw=156`, `stand-lp=144`, `shoryuken-lp=95`, `shoryuken-hp=85`.
  - [x] Phase 10D AWR/AWAC-style weighted imitation: add `--bc-advantage-weighting event-return-v1`, which computes a discounted future combat-event return within each run/episode and converts return-vs-baseline advantage into an extra BC sample weight. Candidate `model/bc-event-return-family-margin-human-10d-v1` combines event weighting, inverse-frequency balancing, 10C family margin, and event-return weighting. Local sanity is not a dramatic jump over 10C-v2, but it changes the mixture (`shoryuken-mp` rises to ~150/5000 at temp 0.8/top-k 8) and is ready for a live comparison gate.
- [ ] Phase 11: move beyond DQN/BC into an event-aware actor-critic line
  - Rationale: repeated DQN reward shaping and Phase 10 BC weighting/margin/return experiments improved diagnostics but still produced stiff live behavior. Phase 11 keeps the proven transition/event data contract and changes the policy architecture/training objective instead of adding more scalar BC weights.
  - Safety rule: combat event rows remain labels/rewards/returns only. Actor-critic features stay the existing DQN feature allowlist unless a later schema-versioned observation change is explicitly planned and validated.
  - [x] Phase 11A actor-critic artifact contract: add `tools/train_actor_critic.py` with `policy=actor-critic`, `actor_critic.schema_version=1`, action-set metadata, existing DQN feature names/scales, independent actor/value MLP payloads, and explicit `algorithm_version=offline-awac-bootstrap-v1`. This is an offline artifact/schema foundation only; probe-side live actor-critic inference is Phase 11D.
  - [x] Phase 11B offline event-return/advantage builder: the same tool now validates paired transition/event logs, derives labels through the Phase 10 BC label path, applies the existing combat-event reward profiles, groups samples by `(run_id, episode_id)`, computes truncated discounted event returns, subtracts a configurable baseline (`zero`, `mean`, or `episode-mean`), and records dataset/reward/action-family diagnostics. Validation on the human paired log produced `23933` labeled samples, `1054` event-reward rows, `reward_sum=92.33`, and zero combat-event validation errors.
  - [x] Phase 11C offline AWAC/AWR trainer: `tools/train_actor_critic.py --training-mode offline-awac` now trains actor logits and a value head from the 11B dataset. The actor loss is masked advantage-weighted cross entropy over the demonstrated action, using clipped/temperature-scaled positive advantages by default; the value loss regresses event returns; masked entropy protects exploration; artifact metadata records AWAC config/stats. A 20-step human-log smoke completed with zero combat-event validation errors and published an `offline-awac-v1` actor-critic artifact under `/tmp/rl-actor-critic-11c-smoke`.
  - [x] Phase 11C-1 local actor distribution diagnostics: actor-critic training now evaluates masked top-1/top-3 label match, target probability, entropy, top action counts, and top action-family counts after training. A 300-step human-log candidate `model/actor-critic-awac-human-11c-v1` stayed numerically stable but failed the local distribution gate: first-5000 eval rows had `top1_match=1811/5000`, `top3_match=4464/5000`, and `top_actions=back=4135/82.7%`. Do not advance this candidate to Phase 11D.
  - [x] Phase 11C-2 action-family balanced AWAC: `tools/train_actor_critic.py` now supports `--awac-sampling family-balanced-v1` and `--awac-family-ratios`, with sampled pool diagnostics for movement-defense, projectile, normal, special, throw, positive-event, and advantage pools. The first balanced 300-step candidate reduced first-5000 `back` top action from `82.7%` to `52.2%`; a more aggressive candidate without fixed movement quota reached `back=48.9%`, `stand-lp=21.9%`, and `fireball-mp=18.7%`. This proves family balancing helps, but it still does not pass the local gate; do not advance to Phase 11D yet.
  - [x] Phase 11C-3 movement-subaction/state-phase balance: AWAC pools now include `move-back`, `move-forward`, `move-guard`, `move-jump-start`, `phase-<mask-phase>`, `action-<action-name>`, `positive-event-nonback`, and `positive-event-nonmovement`, with dataset metadata for movement buckets and mask phases. Candidate `model/actor-critic-awac-movement-balanced-human-11c3-v1` reduced `back` to `12.3%` but shifted collapse to `stand-lp=38.3%`; candidate `model/actor-critic-awac-movement-balanced-human-11c3-v2` added action-level normal pools and reached a much healthier first-5000 local distribution: `fireball-mp=21.9%`, `stand-lp=15.3%`, `back=14.6%`, `stand-mp=10.5%`, `fireball-hp=8.3%`, `forward=7.5%`.
  - [x] Phase 11D probe actor-critic inference: `rl_probe_server.py` now accepts `--policy actor-critic`, loads `actor_critic` artifacts, reads `metadata.awac_config.valid_action_mask` for auto mask selection, applies the shared valid-action mask path, and samples actor logits with `--actor-critic-temperature`, `--actor-critic-top-k`, and optional `--actor-critic-top-p`. Verbose mode reports actor-critic action, mask source, valid count, mask phase, value prediction, and top logits. Local smoke loaded `model/actor-critic-awac-movement-balanced-human-11c3-v2/current.json` and produced a valid masked action/value from a real transition row.
  - [ ] Phase 11E AWAC live gate: compare 11C/11D actor-critic against the best Phase 10 BC candidate using the same MiSTer probe setup. Required reporting: action-family distribution, attack/projectile/throw/guard rates, low-defense response, event journal summary, and subjective live behavior notes.
    - [x] Phase 11E-1 actor-critic spacing prior: add opt-in `--actor-critic-spacing-prior` to the probe inference path. The prior adjusts actor logits at runtime so far-range rows prefer `forward` / fireball over `back`, guard, close normals/throw, and far Shoryuken. It is off by default and intended as a live diagnostic/bridge before range-aware AWAC training. Replay validation on the deterministic 11E live log changed deterministic top actions from `forward=35.7%, stand-lp=29.0%, guard-crouch=16.9%, back=13.2%` to `forward=51.3%, fireball-mp=28.2%, guard-crouch=15.7%, back=4.6%`.
    - [x] Phase 11E-2 threat-aware spacing prior: refine `--actor-critic-spacing-prior` so opponent attack/contact/incoming projectile threat rows stop using the approach bias and instead boost guard/back while penalizing forward and unsafe close attacks. Replay validation on the 11E spacing live log changed 425 threat-row deterministic choices from approach/offense-heavy behavior into `guard-crouch=50.8%, guard-stand=38.6%, back=10.6%`, while keeping no-threat far rows spacing-aware.
  - [ ] Phase 11F online PPO fine-tune plan: only after AWAC has a usable live baseline, collect short on-policy rollouts, join sibling event logs, compute event-primary rewards/advantages, then run clipped PPO actor updates, value updates, entropy regularization, and KL/action-distribution rollback gates.
  - [ ] Phase 11G promotion gates: actor-critic/PPO candidates must keep trainer features clean, pass paired-log validation, avoid single-action collapse, preserve movement/defense competence, and show live event-stat improvement over the current DQN/BC baselines before replacing the deployed model.
- [ ] Phase 12: hierarchical tactical-state policy line
  - Rationale: Phase 9-11 proved that flat `observation -> action` policies can learn pieces of the behavior, but they do not reliably learn the high-level fighting-game rule layer: when to adjust spacing, when to detect/react to threat, when to defend, when to attack first, and when to punish. Phase 12 makes that rule layer explicit before training another policy.
  - Core contract: Phase 12 introduces a tactical intermediate representation derived from existing transition/combat-event logs. Runtime action features remain the existing allowlist unless a later schema-versioned observation change is explicitly planned. Combat event rows remain labels/rewards only.
  - Tactical labels per transition row:
    - `spacing_bucket`: `throw_range`, `close`, `poke`, `fireball`, `too_far`, plus corner-aware variants when available.
    - `self_phase`: `actionable`, `startup`, `active`, `recovery`, `hitstun`, `blockstun`, `airborne`, `knockdown_or_round_boundary`, `unknown`.
    - `opponent_phase`: `neutral`, `startup`, `active`, `recovery`, `jumping`, `fireball_startup`, `projectile_active`, `hitstun`, `blockstun`, `unknown`.
    - `threat_type`: `none`, `low_attack`, `close_attack`, `jump_in`, `incoming_projectile`, `throw_range`, `corner_pressure`, `multi_hit_pressure`, `unknown`.
  ...
  - [x] Phase 12B intent classifier dataset: build a supervised dataset from Phase 12A labels using the existing DQN feature allowlist as inputs and `recommended_intent` as the target. Record label imbalance, ambiguous rows, and ignored/unknown-rate diagnostics.
    - Added `tools/train_tactical_intent.py` dataset mode as part of the training command.
    - Inputs: comma-separated transition logs plus optional sibling combat-event logs.
    - Each sample uses the existing DQN feature allowlist only; combat-event rows are labels/reward diagnostics, not model features.
    - Dataset metadata records transition rows, labeled/skipped rows, intent/spacing/threat/opportunity/confidence distributions, skipped reasons, and event-reward totals by intent.
    - Optional `--labeled-output` emits compact intent-training NDJSON with run/episode/decision ids, recommended intent, confidence, context buckets, and event reward.
  - [x] Phase 12C high-level intent model: train an intent classifier `observation -> intent` with class balancing and confidence diagnostics. It is not allowed to output direct game actions yet. Promotion gate: intent confusion matrix must pass manual review on held-out live logs.
    - `tools/train_tactical_intent.py` trains a small MLP classifier over the same DQN feature vector used by the existing learner.
    - The artifact schema is `policy=tactical` with a nested `tactical_intent` payload; it predicts only intent, and `tools/rl_probe_server.py` still decodes `intent + observation -> action` through the conservative tactical decoder.
    - The trainer supports `--balanced-loss` plus `--class-weight-power` so class balancing can be tuned without letting rare intents dominate.
    - Output artifacts are `tactical-intent-vN.json`, `actor-vN.json`, and `current.json`.
  - [x] Phase 12D intent-conditioned action decoder: map `intent + observation -> action` using a conservative rule decoder first, then an optional learned low-level policy. Examples: `approach -> forward`, `incoming_projectile -> guard/parry/jump/back`, `anti_air -> shoryuken/crouch-hp`, `poke -> crouch-mk/stand-mp/fireball`, `punish -> high-value starter/throw`.
    - Added the first conservative live decoder inside `tools/rl_probe_server.py` behind `--policy tactical`.
    - The live decoder uses the same tactical buckets as Phase 12A where the UDP observation payload supports them: spacing, corner context, self phase, opponent phase, projectile/close/jump-in threat, opportunity, intent, reason, confidence, and decoded action.
    - Because the live OBS payload is smaller than transition rows, low-attack and event-derived punish windows remain Phase 12A/12B offline labels until the runtime observation schema is explicitly expanded.
  - [x] Phase 12E hierarchical probe mode: add a probe-server policy that logs `intent`, `intent_confidence`, `intent_reason`, and final action for every decision. Live gate compares tactical behavior, not only action distribution: spacing adjustment, threat reaction, punish attempts, anti-air attempts, and low-defense response.
    - `--policy tactical` is live-testable without a model artifact.
    - `--verbose` prints `tactical_intent`, `tactical_reason`, `tactical_conf`, `tactical_spacing`, `tactical_self`, `tactical_opp`, `tactical_threat`, `tactical_opportunity`, and final action diagnostics per observation.
    - Local replay smoke on Phase 11E and human logs showed mixed tactical actions rather than a single-action collapse; on the 11E spacing-threat log, first 1000 decoded actions were `guard-crouch=504`, `neutral=181`, `back=73`, `forward=68`, `fireball-mp=67`, `fireball-hp=44`, plus jump/poke actions.
  - [ ] Phase 12E-1 tactical live gate: run `--policy tactical` on MiSTer and judge whether high-level behavior improves: far/no-threat rows should approach or zone, incoming projectile/close threat rows should defend, self-attacking rows should stay neutral instead of buffering guard, and close/no-threat rows should poke/throw/pressure instead of standing still.
  - [x] Phase 12F event-feedback refinement: use combat-event outcomes to score intents, not only actions. Examples: `approach` followed by getting hit is bad approach timing; `low_guard` blocking low attacks is successful defense; `anti_air` hitting a jump-in is successful reaction; `punish` landing after opponent recovery is successful punish.
    - The Phase 12 trainer joins optional combat-event logs through `rl_combat_event_training.py` and records event-reward rows/sums by intent in metadata.
    - This is diagnostic feedback for intent quality; it does not add combat-event fields to the runtime feature vector.
  - [x] Phase 12G promotion gates: a hierarchical candidate must improve live tactical stats over Phase 11, avoid a single-intent collapse, preserve movement/defense competence, and keep transition/event training compatibility clean before replacing existing DQN/BC/actor-critic baselines.
