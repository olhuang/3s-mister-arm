...
- [ ] Phase 11: move beyond DQN/BC into an event-aware actor-critic line
  ...
  - [ ] Phase 11C-2 action-family balanced AWAC: repair the 11C collapse before live inference by balancing actor batches or actor loss across movement-defense, projectile, normal, special, throw, and positive-event rows. The gate is local first: top action-family distribution must beat Phase 10 BC candidates and avoid a single movement action dominating before `--policy actor-critic` is added to the probe.
