...
- [ ] Combat event attribution Phase 5: implement throw event tracking so close guard failures can distinguish thrown/tech/whiff/unknown from strike or chip damage
  ...
  - [x] Phase 5C/5D live-refinement prep: throw starts now include policy/engine throw attempts, so out-of-range throws can become `CTR W`; simultaneous/contested throw or throw-escape routine evidence is kept out of `CTR T` and conservatively finalized as `CTR U`; normal and back throw success still use caught-state evidence
