...
- [ ] Combat event attribution Phase 9: add opt-in trainer use of high-confidence event labels only after move-family validation passes
  ...
  - [ ] Phase 9F data gate: collect paired logs that deliberately include Ryu/Ken `crouch-lk`, `crouch-mk`, and `crouch-hk` hitting and being blocked. The current 30-round Ryu/Ken Phase 9 dataset produced `low_rows=0` in the Phase 9F smoke, so low-defense shaping cannot affect a candidate until the dataset contains low-source events.
