...
- [ ] Combat event attribution Phase 9: add opt-in trainer use of high-confidence event labels only after move-family validation passes
  ...
  - [ ] Phase 9B trainer reader: add explicit `--combat-event-logs`, `--combat-event-training-mode off|reward-shaping|prefer-event-action`, and `--combat-event-reward-profile safe-v1` flags. Defaults remain off and must preserve bit-for-bit training behavior without event flags.
  - [ ] Phase 9B reward-shaping MVP: join event rows to transition rows by `(run_id, episode_id, start_decision_id)` and apply small, opt-in event reward adjustments while keeping DQN feature names unchanged. Event fields are labels/metadata, not model inputs.
