...
#define RL_COMBAT_ATTACK_START_DEBOUNCE_FRAMES 24u
...
static void RLSession_UpdateCombatEventDebugStats(void) {
    ...
    remote_debug.combat_contact_match_attack_self_count = stats->contact_match_attack_self_count;
    remote_debug.combat_contact_match_attack_opp_count = stats->contact_match_attack_opponent_count;
    remote_debug.combat_contact_match_projectile_self_count = stats->contact_match_projectile_self_count;
    remote_debug.combat_contact_match_projectile_opp_count = stats->contact_match_projectile_opponent_count;
    remote_debug.combat_contact_match_throw_self_count = stats->contact_match_throw_self_count;
    ...
    remote_debug.combat_punish_candidate_self_count = stats->punish_candidate_self_count;
    remote_debug.combat_punish_candidate_opp_count = stats->punish_candidate_opponent_count;
    remote_debug.combat_punish_whiff_self_count = stats->punish_whiff_self_count;
    remote_debug.combat_punish_whiff_opp_count = stats->punish_whiff_opponent_count;
    remote_debug.combat_punish_interrupted_self_count = stats->punish_interrupted_self_count;
    remote_debug.combat_punish_interrupted_opp_count = stats->punish_interrupted_opponent_count;
    remote_debug.combat_punish_source_attack_self_count = stats->punish_source_attack_self_count;
    remote_debug.combat_punish_source_attack_opp_count = stats->punish_source_attack_opponent_count;
    remote_debug.combat_punish_source_projectile_self_count = stats->punish_source_projectile_self_count;
    remote_debug.combat_punish_source_projectile_opp_count = stats->punish_source_projectile_opponent_count;
    remote_debug.combat_punish_source_throw_self_count = stats->punish_source_throw_self_count;
    remote_debug.combat_punish_source_throw_opp_count = stats->punish_source_throw_opponent_count;
    remote_debug.combat_punish_finalized_candidate_self_count = stats->punish_finalized_candidate_self_count;
    remote_debug.combat_punish_finalized_candidate_opp_count = stats->punish_finalized_candidate_opponent_count;
    remote_debug.combat_punish_active_candidate_self_count = stats->punish_active_candidate_self_count;
    remote_debug.combat_punish_active_candidate_opp_count = stats->punish_active_candidate_opponent_count;
    remote_debug.combat_punish_finalized_whiff_self_count = stats->punish_finalized_whiff_self_count;
    remote_debug.combat_punish_finalized_whiff_opp_count = stats->punish_finalized_whiff_opponent_count;
    remote_debug.combat_punish_finalized_interrupted_self_count = stats->punish_finalized_interrupted_self_count;
    remote_debug.combat_punish_finalized_interrupted_opp_count = stats->punish_finalized_interrupted_opponent_count;
    remote_debug.combat_punish_active_whiff_self_count = stats->punish_active_whiff_self_count;
    remote_debug.combat_punish_active_whiff_opp_count = stats->punish_active_whiff_opponent_count;
    remote_debug.combat_punish_active_interrupted_self_count = stats->punish_active_interrupted_self_count;
    remote_debug.combat_punish_active_interrupted_opp_count = stats->punish_active_interrupted_opponent_count;
...
static bool RLSession_BuildEngineAttributionForSideEx(const RLDecisionLedgerEntry* entry,
                                                      const RLObservationV1* obs,
                                                      RLCombatEventSide side,
                                                      RLEngineAttribution* attribution,
                                                      bool allow_attack_counter_edge) {
    ...
    if (side == RL_COMBAT_EVENT_SIDE_SELF) {
        ...
        attack_edge = obs->self_attack_started || obs->self_attack_code_changed ||
                      (allow_attack_counter_edge && obs->self_attack_counter_started);
        ...
        attack_edge = obs->opp_attack_started || obs->opp_attack_code_changed ||
                      (allow_attack_counter_edge && obs->opp_attack_counter_started);
...
static bool RLSession_BuildEngineAttributionForSide(const RLDecisionLedgerEntry* entry,
                                                    ...
                                                    RLCombatEventSide side,
                                                    RLEngineAttribution* attribution) {
    return RLSession_BuildEngineAttributionForSideEx(entry, obs, side, attribution, true);
}

static bool RLSession_BuildEngineStartAttributionForSide(const RLDecisionLedgerEntry* entry,
                                                         const RLObservationV1* obs,
                                                         RLCombatEventSide side,
                                                         RLEngineAttribution* attribution) {
...
static void RLSession_UpdateCombatAttackStartLifecycleFlags(RLCombatAttackEventStart* start) {
    if (start == NULL) {
        return;
    }

    const u16 lifecycle_action_id = (start->engine_action_id != RL_POLICY_ACTION_NEUTRAL) ? start->engine_action_id
                                                                                         : start->policy_action_id;
    const u16 lifecycle_sub_action_id = (start->engine_action_id != RL_POLICY_ACTION_NEUTRAL)
                                            ? start->engine_sub_action_id
...
static void RLSession_FillCombatAttackStart(RLCombatAttackEventStart* start,
    ...
    if (side == RL_COMBAT_EVENT_SIDE_SELF) {
        ...
        RLSession_UpdateCombatAttackStartLifecycleFlags(start);
    } else if (side == RL_COMBAT_EVENT_SIDE_OPPONENT) {
...
static bool RLSession_CombatAttackStartUsesEngineOwner(const RLDecisionLedgerEntry* entry,
                                                       RLCombatEventSide side) {
    if (entry == NULL) {
        return false;
    }
...
static void RLSession_ApplyEngineAttributionToAttackStart(RLCombatAttackEventStart* start,
                                                          const RLEngineAttribution* attribution) {
    if (start == NULL || attribution == NULL || attribution->label_source == RL_DEMO_ATTRIBUTION_NONE) {
        return;
    }

    start->engine_action_id = attribution->action_id;
    start->engine_sub_action_id = attribution->sub_action_id;
    start->engine_routine_1 = attribution->routine_1;
    start->engine_routine_2 = attribution->routine_2;
    start->engine_current_attack = attribution->current_attack;
    start->engine_lag_frames = attribution->lag_frames;
    start->engine_kind_of_waza = attribution->kind_of_waza;
...
static void RLSession_MaybeStartCombatAttackEvent(RLDecisionLedgerEntry* entry,
    ...
    RLEngineAttribution engine_start_attribution;
    const bool engine_owned_start = RLSession_CombatAttackStartUsesEngineOwner(entry, side);

    if (entry == NULL || obs == NULL) {
        return;
    }
    memset(&engine_start_attribution, 0, sizeof(engine_start_attribution));

    if (engine_owned_start) {
        if (!RLSession_BuildEngineStartAttributionForSide(entry, obs, side, &engine_start_attribution)) {
            return;
        }
    } else if (!RLSession_ObservationAttackStartedForSide(obs, side)) {
        return;
    }
    ...
    if (engine_owned_start) {
        RLSession_ApplyEngineAttributionToAttackStart(&start, &engine_start_attribution);
        RLSession_UpdateCombatAttackStartLifecycleFlags(&start);
...
static bool RLSession_CombatActionCanUseDamageFallback(u16 policy_action_id) {
    switch (policy_action_id) {
    case RL_POLICY_ACTION_RYU_SHORYUKEN:
    case RL_POLICY_ACTION_RYU_TATSU:
...
static bool RLSession_CombatSideHasTargetDamageEvidence(const RLObservationV1* obs,
                                                        RLCombatEventSide side,
                                                        s16 self_hp_delta,
                                                        s16 opp_hp_delta) {
    if (obs == NULL) {
    ...
    if (side == RL_COMBAT_EVENT_SIDE_SELF) {
        return opp_hp_delta > 0 || obs->delta_opp_stun > 0;
    }
    if (side == RL_COMBAT_EVENT_SIDE_OPPONENT) {
        return self_hp_delta > 0 || obs->delta_self_stun > 0;
    }
    return false;
}

static void RLSession_MaybeStartCombatAttackEventFromDamage(RLDecisionLedgerEntry* entry,
                                                            const RLObservationV1* obs,
                                                            RLCombatEventSide side,
                                                            s16 self_hp_delta,
                                                            s16 opp_hp_delta) {
    ...
    if (!RLSession_CombatSideHasTargetDamageEvidence(obs, side, self_hp_delta, opp_hp_delta)) {
        return;
    }
    ...
    RLSession_FillCombatAttackStart(&start, entry, obs, side);
    RLSession_ApplyEngineAttributionToAttackStart(&start, &attribution);
    RLSession_UpdateCombatAttackStartLifecycleFlags(&start);
    if (start.run_id == 0 || start.episode_id == 0 || start.side == RL_COMBAT_EVENT_SIDE_NONE) {
        return;
    }
...
static void RLSession_FillCombatContactMatchUpdate(RLCombatContactMatchUpdate* update,
                                                   const RLDecisionLedgerEntry* entry,
                                                   const RLObservationV1* obs,
...
static void RLSession_UpdateCombatContactMatches(RLDecisionLedgerEntry* entry,
                                                 const RLObservationV1* obs,
                                                 s16 self_hp_delta,
                                                 s16 opp_hp_delta) {
    RLCombatContactMatchUpdate update;

    if (entry == NULL || obs == NULL) {
        return;
    }

    RLSession_FillCombatContactMatchUpdate(&update,
                                           entry,
                                           obs,
                                           RL_COMBAT_EVENT_SIDE_SELF,
                                           self_hp_delta,
                                           opp_hp_delta);
    RLCombatEvent_RecordContactMatch(&update);
    RLSession_FillCombatContactMatchUpdate(&update,
                                           entry,
                                           obs,
                                           RL_COMBAT_EVENT_SIDE_OPPONENT,
...
static void RLSession_AccumulateCombatSpan(RLDecisionLedgerEntry* entry,
    ...
    RLSession_MaybeStartCombatAttackEventFromDamage(entry, obs, RL_COMBAT_EVENT_SIDE_SELF, self_hp_delta, opp_hp_delta);
    RLSession_MaybeStartCombatAttackEventFromDamage(entry, obs, RL_COMBAT_EVENT_SIDE_OPPONENT, self_hp_delta, opp_hp_delta);
    RLSession_UpdateCombatAttackEvents(entry, obs, self_hp_delta, opp_hp_delta);
    RLSession_UpdateCombatContactMatches(entry, obs, self_hp_delta, opp_hp_delta);
    RLSession_MaybeAttributeEngineActionForSide(entry, obs, RL_COMBAT_EVENT_SIDE_SELF);
    RLSession_MaybeAttributeEngineActionForSide(entry, obs, RL_COMBAT_EVENT_SIDE_OPPONENT);
}

static u32 RLSession_CombatFrameContextDecisionId(void) {
    if (active_ledger_entry != NULL && active_ledger_entry->valid && active_ledger_entry->active) {
        return active_ledger_entry->decision_id;
    }
    if (remote_debug.next_decision_id > 0) {
        return remote_debug.next_decision_id - 1u;
    }
    return 0;
}

static bool RLSession_FillCombatFrameContext(RLDecisionLedgerEntry* entry, const RLObservationV1* obs) {
    if (entry == NULL || obs == NULL || remote_debug.run_id == 0 || remote_debug.episode_id == 0) {
        return false;
    }
    if (!RLSession_CanRecordDemoInput()) {
        return false;
    ...
    memset(entry, 0, sizeof(*entry));
    entry->valid = true;
    entry->run_id = remote_debug.run_id;
    entry->episode_id = remote_debug.episode_id;
    entry->decision_id = RLSession_CombatFrameContextDecisionId();
    entry->round_num = Round_num;
    entry->mode_type = (u8)Mode_Type;
    entry->play_mode = Play_Mode;
    entry->start_self_hp = obs->self_hp;
    entry->start_opp_hp = obs->opp_hp;
    entry->final_self_hp = obs->self_hp;
    entry->final_opp_hp = obs->opp_hp;
    entry->agent_character_id = My_char[RLSession_AgentPlayerIndex()];
    entry->opponent_character_id = My_char[RLSession_OpponentPlayerIndex()];
    entry->obs_frame = remote_debug.frame_id;
    entry->target_frame = remote_debug.frame_id;
    entry->execution_frame_actual = remote_debug.frame_id;
    entry->model_version_expected = remote_debug.model_version_current;
    entry->model_version_requested = remote_debug.model_version_current;
    entry->model_version_executed = remote_debug.model_version_current;
    entry->terminal_reason = 3;
    RLSession_CaptureObservationSpacing(entry, obs);
    return true;
}

static void RLSession_UpdateCombatFrameContext(const RLObservationV1* obs,
                                               s16 self_hp_delta,
                                               s16 opp_hp_delta) {
    RLDecisionLedgerEntry context;

    if (!RLSession_FillCombatFrameContext(&context, obs)) {
        return;
    }

    RLSession_AccumulateAttackSignals(&context, obs);
    RLSession_AccumulateCombatSpan(&context, obs, self_hp_delta, opp_hp_delta);
}
...
void RLSession_OnObservationFrameEnd(const RLObservationV1* obs) {
    ...
    if (active_entry != NULL) {
        ...
        RLSession_UpdateCombatFrameContext(obs, self_hp_delta, opp_hp_delta);
    }
