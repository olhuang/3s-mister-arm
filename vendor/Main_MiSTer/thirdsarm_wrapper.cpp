...
int read_runtime_rl_opponent_default()
{
	char value[64] = {};
	if (!read_runtime_config_value("rl-opponent-mode", value, sizeof(value))) return kRLOpponentCPU;

	if (!strcasecmp(value, "human")) return kRLOpponentHuman;
	return kRLOpponentCPU;
}

static const char *runtime_rl_opponent_config_value(int mode)
{
	switch (mode)
	{
	case kRLOpponentHuman: return "human";
	default: return "cpu";
	}
}

static const char *runtime_rl_opponent_mode_name(int mode)
{
	switch (mode)
	{
	case kRLOpponentHuman: return "human";
...
bool write_runtime_rl_opponent_default(int mode)
{
	char path[PATH_MAX] = {};
	char temp_path[PATH_MAX] = {};
	snprintf(path, sizeof(path), "%s/config", kRuntimeHome);
	...
	if (in)
	{
		while (fgets(line, sizeof(line), in))
		{
			...
			if (equals)
			{
				*equals = 0;
				trim_in_place(cursor);
				if (!strcasecmp(cursor, "rl-opponent-mode"))
				{
					fprintf(out, "rl-opponent-mode = %s\n", runtime_rl_opponent_config_value(mode));
					wrote_value = true;
					continue;
				}
			}
...
int read_runtime_rl_network_default()
{
	char value[64] = {};
	if (!read_runtime_config_value("rl-network", value, sizeof(value))) return kRLNetworkOff;

	if (!strcasecmp(value, "on") || !strcasecmp(value, "true") || !strcasecmp(value, "1"))
		return kRLNetworkOn;
	return kRLNetworkOff;
}

static const char *runtime_rl_network_config_value(int mode)
{
	switch (mode)
	{
	case kRLNetworkOn: return "on";
	default: return "off";
	}
}

static const char *runtime_rl_network_mode_name(int mode)
{
	return runtime_rl_network_config_value(mode);
}

bool write_runtime_rl_network_default(int mode)
{
	char path[PATH_MAX] = {};
	char temp_path[PATH_MAX] = {};
	snprintf(path, sizeof(path), "%s/config", kRuntimeHome);
	...
	if (in)
	{
		while (fgets(line, sizeof(line), in))
		{
			...
			if (equals)
			{
				*equals = 0;
				trim_in_place(cursor);
				if (!strcasecmp(cursor, "rl-network"))
				{
					fprintf(out, "rl-network = %s\n", runtime_rl_network_config_value(mode));
					wrote_value = true;
					continue;
				}
			}
...
void append_runtime_launch_args(std::vector<char *> &child_argv, int argc, char *argv[])
{
	...
	if (g_wrapper_rl_agent_mode == kRLAgentPlayer1 || g_wrapper_rl_agent_mode == kRLAgentPlayer2)
	{
		...
		if (g_wrapper_rl_network_mode == kRLNetworkOn)
		{
...
void poll_status_changes(pid_t child)
{
	...
	uint32_t rl_opponent = user_io_status_get("[29]");
	if (rl_opponent != prev_rl_opponent) {
		prev_rl_opponent = rl_opponent;
		int target = (int)rl_opponent;
		if (target >= kRLOpponentMenuCount) target = kRLOpponentCPU;
		if (target != g_wrapper_rl_opponent_mode) {
			write_runtime_rl_opponent_default(target);
			g_wrapper_rl_opponent_mode = target;
			// Opponent mode is a launch-time RL setting. Persist now and apply on restart.
		}
	}
	...
	uint32_t rl_network = user_io_status_get("[49]");
	if (rl_network != prev_rl_network) {
		prev_rl_network = rl_network;
		int target = (int)rl_network;
		if (target >= kRLNetworkMenuCount) target = kRLNetworkOff;
		if (target != g_wrapper_rl_network_mode) {
			write_runtime_rl_network_default(target);
			g_wrapper_rl_network_mode = target;
			// Network mode is launch-time. IP and ports stay in games/3s-arm/config.
		}
	}
